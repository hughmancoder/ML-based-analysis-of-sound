...
from features import (
    load_audio_stereo, ensure_duration, calc_fft_hop, mel_stereo2_from_stereo
)

class MelDataset(Dataset):
    """
    Expects a manifest CSV with columns: filepath,label
    - If cache_root is set, will try to load/save (2,H,W) mel .npy files there.
    """
    ...
    def _compute_mel(self, wav_path: Path):
        ...
        stereo = ensure_duration(stereo, self.sr, self.duration_s) # (2, T*)
        mel = mel_stereo2_from_stereo(stereo, self.sr, self.n_fft, self.hop,
                                      self.n_mels, self.fmin, self.fmax)     # (2,H,W)
