...
from src.classes import ALL_CLASSES
...
def build_multi_manifest_dataloaders(
    manifest_csvs: Union[str, Path, Sequence[Union[str, Path]]],
    val_frac: float = 0.15,
    batch_size: int = 32,
    num_workers: int = 2,
    pin_memory: bool = False,
    ...
    """
    Build dataloaders by concatenating multiple single-class manifests.
    Keeps a consistent label mapping across all datasets.
    """
    if label_to_idx is None:
        raise ValueError("label_to_idx must be provided when combining manifests.")

    if isinstance(manifest_csvs, (str, Path)):
        manifest_list = [manifest_csvs]
    else:
        manifest_list = list(manifest_csvs)

    if not manifest_list:
        raise ValueError("manifest_csvs must contain at least one manifest path.")

    datasets = [
        SingleClassMelNpyDataset(str(csv_path), label_to_idx=label_to_idx, per_example_norm=True)
        for csv_path in manifest_list
    ]
    full_ds = ConcatDataset(datasets)

    N = len(full_ds)
    if N == 0:
        raise ValueError("Combined dataset is empty; check manifest paths and contents.")
...
def load_pretrained_weights_for_finetune(model: nn.Module, ckpt_path: Path) -> Tuple[List[str], List[str]]:
    """
    Load weights from a checkpoint into `model`, skipping layers whose shapes do not match.
    Returns a tuple of (loaded_keys, skipped_keys).
    """
    state = load_model_state(str(ckpt_path))
...
def single_class_train_loop(
    ...
    save_best_stamped: bool = False,       # also save best_eXXXX_accYY.YY.pt
) -> Dict[str, Any]:
...
def fine_tune_single_class(
    ...
    """
    Fine-tune a single-class classifier by combining multiple manifest CSVs
    (e.g. IRMAS + Chinese instruments) while reusing a pretrained checkpoint.
    """
    ...
    classes_seq = list(classes) if classes is not None else list(ALL_CLASSES)
    label_to_idx = {str(name).strip().lower(): idx for idx, name in enumerate(classes_seq)}
    if len(label_to_idx) != len(classes_seq):
        raise ValueError("Duplicate class names detected in provided classes sequence.")

    train_loader, val_loader, _ = build_multi_manifest_dataloaders(
        manifest_csvs=manifest_csvs,
        val_frac=val_frac,
    ...
    pretrained_path = Path(pretrained_ckpt) if pretrained_ckpt is not None else None

    if resume_path is not None and resume_path.exists():
        print(f"Resuming fine-tune from {resume_path}")
        ckpt = load_checkpoint(
            resume_path, device, model, optimizer, scheduler, scaler, current_label_to_idx=label_to_idx
        )
        start_epoch   = int(ckpt.get("epoch", 0)) + 1
        history       = ckpt.get("history", history)
        best_val_acc  = float(ckpt.get("best_val_acc", best_val_acc))
        ...
        if pretrained_path is not None and pretrained_path.exists():
            loaded, skipped = load_pretrained_weights_for_finetune(model, pretrained_path)
            print(f"Loaded pretrained weights from {pretrained_path}")
            if skipped:
                print(f"Skipped layers (shape mismatch): {skipped}")
        else:
            print("No pretrained checkpoint provided; training from scratch on combined manifests.")
